...
def generate_regions(fasta_index_file, size, output_bed=None, chunks=False, chromosomes=None, bed_dir=None):
    ...
    with open(fasta_index_file, "r") as fasta_index:
        for line in fasta_index:
            ...
            while region_start < chrom_length:
                ...
                if bed_dir is not None:
                    region = str(ceil(region_end / region_size))
                    file_path = f"{bed_dir}/{chrom_name}.region.{region}.bed"
                    with open(file_path, "w") as f:
                        f.write("\t".join([chrom_name, start, end]))
                elif output_bed is not None:
                    line = f"{chrom_name}:{start}-{end}"
